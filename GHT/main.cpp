...
//shape feature based experiment
...
list<score> get_top_num(RTable test, vector<Shape> pool, int num);
list<string>get_fifty_semantic(string semantic, list<int> &label);
vector<std::string> split(const string& s, char delimiter);
vector<geometric> geometry;
vector<geometric> geometry_query;
list<string> get_top_num_location(geometric test, int num);
list<string> get_top_num_area(geometric test, int num);
list<string> get_top_num_ratio(geometric test, int num);
void loadgeometry();
...
int main()
{
	...
	for (int i = 0; i < test.size() - 1; i++)
	{
		...
		*/
		list <score> top_ten = get_top_num(test[i].r_table, extractor, 100);
		list <int>labels;
		list<string> semantic_fifty = get_fifty_semantic(test[i].name, labels);
		list<string> location_fifty = get_top_num_location(geometry_query[i], 100);
		int k = 0;
		int total_score = 0;
		...
		/*
		for (list<string>::iterator iterPos = location_fifty.begin(); iterPos != location_fifty.end(); iterPos++)
		{
			string name = *iterPos;
			int count = 0;
		...
		{
			string name = *iterPos;
			for (list<score>::iterator iterPos2 = top_ten.begin(); iterPos2 != top_ten.end(); iterPos2++)
			{
				if (*iterPos == iterPos2->name)
				{
					resorted_list.push_back(*iterPos2);
					top_ten.erase(iterPos2);
					break;
				}
				//if (resorted_list.size() == 20)
					//break;
		...
		for (int j = resorted_list.size(); j < 100; j++)
		{
			score temp = *top_ten.begin();
			top_ten.pop_front();
			resorted_list.push_back(temp);
		}
		top_ten.clear();
		for (int j = 0; j < 100; j++)
		{
			top_ten.push_back(*resorted_list.begin());
			resorted_list.pop_front();
		}
		*/
...
list<score> get_top_num(RTable test, vector<Shape> pool, int num)
{
	...
	for (int i = 0; i < pool.size(); i++)
	{
		...
		if (top_ten.size() == (num + 1))
			top_ten.pop_back();
		if (top_ten.size() < num && insert == false)
		{
			score temp;
			temp.name = pool[i].name;
			temp.label = pool[i].label;
...
void loadgeometry()
{
	ifstream image_pool("geo_features_pool.txt");
	while (!image_pool.eof())
	{
		geometric temp;
		image_pool >> temp.name >> temp.label >> temp.x >> temp.y >> temp.top_x >> temp.top_y >> temp.bottom_x >> temp.bottom_y >> temp.area >> temp.ratio;
		geometry.push_back(temp);
	}
	image_pool.close();
	ifstream query("geo_test.txt");
	while (!query.eof())
	{
		geometric temp;
		query >> temp.name >> temp.label >> temp.x >> temp.y >> temp.top_x >> temp.top_y >> temp.bottom_x >> temp.bottom_y >> temp.area >> temp.ratio;
		geometry_query.push_back(temp);
	}
	query.close();
}
...
list<string> get_top_num_location(geometric test, int num)
{
	...
	score temp;
	double distance = sqrt(pow((double)abs(test.x - geometry[0].x), 2) + pow((double)abs(test.y - geometry[0].y), 2));
	temp.name = geometry[0].name;
	temp.label = geometry[0].label;
	temp.distance = distance;
	temp.index = 0;
	top_ten.insert(top_ten.begin(), temp);
	for (int i = 1; i < geometry.size(); i++)
	{
		double distance = sqrt(pow((double)abs(test.x - geometry[i].x), 2) + pow((double)abs(test.y - geometry[i].y), 2));
		bool insert = false;
		for (list<score>::iterator iterPos = top_ten.begin(); iterPos != top_ten.end(); iterPos++)
		{
			if (iterPos->distance > distance)
			{
				score temp;
				temp.name = geometry[i].name;
				temp.label = geometry[i].label;
				temp.distance = distance;
				temp.index = i;
				top_ten.insert(iterPos, temp);
				insert = true;
	...
	for (list<score>::iterator iterPos = top_ten.begin(); iterPos != top_ten.end(); iterPos++)
	{
		top_list.push_back(iterPos->name);
	}
	return top_list;
}
...
list<string> get_top_num_area(geometric test, int num)
{
	...
	list<score> top_ten;
	score temp;
	double distance = abs((double)(test.area - geometry[0].area) / 5.0);
	temp.name = geometry[0].name;
	temp.label = geometry[0].label;
	temp.distance = distance;
	temp.index = 0;
	...
	for (int i = 1; i < geometry.size(); i++)
	{
		double distance = abs((double)(test.area - geometry[i].area) / 5.0);
		bool insert = false;
		for (list<score>::iterator iterPos = top_ten.begin(); iterPos != top_ten.end(); iterPos++)
		{
			if (iterPos->distance > distance)
...
list<string> get_top_num_ratio(geometric test, int num)
{
	...
	list<score> top_ten;
	score temp;
	double distance = (double)abs((test.ratio - geometry[0].ratio)*100.0);
	temp.name = geometry[0].name;
	temp.label = geometry[0].label;
	temp.distance = distance;
	temp.index = 0;
	...
	for (int i = 1; i < geometry.size(); i++)
	{
		double distance = (double)abs((test.ratio - geometry[i].ratio)*100.0);
		bool insert = false;
		for (list<score>::iterator iterPos = top_ten.begin(); iterPos != top_ten.end(); iterPos++)
		{
			if (iterPos->distance > distance)
