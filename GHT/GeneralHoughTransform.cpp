...
std::vector<std::vector<cv::Vec2f>> GeneralHoughTransform::getRTable()
{
	return m_RTable;
}

cv::Vec2f GeneralHoughTransform::getOrigin()
{
	return m_origin;
}
...
void GeneralHoughTransform::findOrigin() {
	...
	for(int j=0 ; j<m_templateImage.rows ; j++) {
		...
		for(int i=0 ; i<m_templateImage.cols ; i++)
			if(data[i]==Vec3b(0,0,0)) { // If it's a red pixel...
				if (first == false)
				{
					firstPoint = Vec2f(i, j);
					first = true;
				}
				else
	...
	float x = m_origin[0];
	float y = m_origin[1];
	m_origin = Vec2f((float)((int)(firstPoint[0] + x) / 2), (float)((int)(firstPoint[1] + y) /2));
}
